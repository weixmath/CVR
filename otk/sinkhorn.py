...
def multihead_attn(input, weight, mask=None, eps=1.0, return_kernel=False,
    ...
    # K = torch.tensordot(torch.nn.functional.normalize(input,dim=-1), torch.nn.functional.normalize(weight,dim=-1), dims=[[-1], [-1]])
    K = torch.tensordot(normalize(input), normalize(weight), dims=[[-1], [-1]])
