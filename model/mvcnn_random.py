...
class view_GCN(Model):
    ...
    def forward(self,x,rand_view_num,N):
        y = self.net_1(x)
        y = y.squeeze()
        y0 = my_pad_sequence(sequences=y, view_num=rand_view_num, N=N, max_length=self.num_views, padding_value=0)
        pooled_view = y0.max(1)[0]
        pooled_view = self.net_2(pooled_view)
        return pooled_view