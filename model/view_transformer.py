...
class view_GCN(Model):
    def __init__(self,name, model, nclasses=40, cnn_name='resnet18', num_views=20):
        ...
        self.dim = 512
        self.encoder1 = TransformerEncoderLayer(d_model=self.dim,nhead=4)
        self.encoder2 = TransformerEncoderLayer(d_model=self.dim,nhead=4)
        self.ff = PositionWiseFeedForward(d_model=512, d_ff=512)
        self.cls = nn.Sequential(
            nn.Linear(512, 256),
            nn.Dropout(),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.cls2 = nn.Linear(256,self.nclasses)
    def forward(self,x):
        y = self.net_1(x)
        y = y.view((int(x.shape[0] / self.num_views), self.num_views, -1))
        y0 = self.encoder1(src=y.transpose(0,1), src_key_padding_mask=None, pos=None)
        y0 = y0.transpose(0,1)
        y1 = self.otk_layer(y0)
        y11 = self.ff(y1)
        pos0 = normalize(self.coord_encoder(y11))
        pos = self.coord_decoder(pos0)
        y2 = self.encoder2(src=y11.transpose(0,1),src_key_padding_mask=None, pos=pos.transpose(0,1))
        y2 =y2.transpose(0,1)
