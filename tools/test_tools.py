...
class ModelNetTrainer(object):
    ...
    def train(self, n_epochs):
        ...
        scheduler = CosineAnnealingLR(self.optimizer,T_max=((len(self.train_loader.dataset.rand_view_num)//20)*60),eta_min=0)
        for epoch in range(n_epochs):
            ...
            NN = []
            label = []
            file_name = self.train_loader.dataset.filepaths
            file = open('test2/list.txt', 'w')
            for fp in file_name:
                file.write(str(fp))
                file.write('\n')
            file.close()
            ...
            for i, data in enumerate(self.train_loader):
                ...
                with torch.no_grad():
                    out_data,cos_sim,cos_sim2,pos,y0,y1= self.model(in_data,rand_view_num,N)
                    ...
                    y00.append(y0.detach().cpu().numpy())
                    y11.append(y1.detach().cpu().numpy())
                    cos_loss22.append(cos_sim2.detach().cpu().numpy())
                    label.append(target.detach().cpu().numpy())
                    NN.append(rand_view_num.detach().cpu().numpy())
                ...
                if epoch>0:
                    scheduler.step()
                # self.optimizer.zero_grad()
                log_str = 'epoch %d, step %d: train_loss %.3f;cos_loss %.3f;cos_loss%.3f; train_acc %.3f' % (epoch + 1, i + 1, loss,cos_loss2,pos_loss, acc)
                if (i + 1) % 1 == 0:
                    print(log_str)
            i_acc += i
            np.save('test2/y0.npy', np.concatenate(y00,axis=0))
            np.save('test2/y1.npy', np.concatenate(y11,axis=0))
            np.save('test2/NN.npy', np.concatenate(NN, axis=0))
            np.save('test2/label.npy', np.concatenate(label, axis=0))
            np.save('test2/cos_loss22.npy', np.concatenate(cos_loss22,axis=0))
            # evaluation
            # if (epoch + 1) % 1 == 0:
            #     with torch.no_grad():
